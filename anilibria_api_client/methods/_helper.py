...
async def validate_filters(params: Release) -> dict[str, Any]:
    ...
    if "types" in filters and filters["types"] is not None:
        if not isinstance(filters["types"], list) or not all(
            isinstance(t, ContentType) for t in filters["types"]
        ):
    ...
    if "seasons" in filters and filters["seasons"] is not None:
        if not isinstance(filters["seasons"], list) or not all(
            isinstance(s, Seasons) for s in filters["seasons"]
        ):
    ...
    if "age_ratings" in filters and filters["age_ratings"] is not None:
        if not isinstance(filters["age_ratings"], list) or not all(
    ...
    if (
        ...
        if not isinstance(filters["production_statuses"], list) or not all(
            ...
            raise ValueError(
                "production_statuses должен быть списком ProductionStatusesType"
            )
        validated_filters["f[production_statuses]"] = [
            p.value for p in filters["production_statuses"]
        ]
